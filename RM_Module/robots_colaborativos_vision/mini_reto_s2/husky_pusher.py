...
class CorridorWorld:
    ...
    Las cajas comienzan sobre la trayectoria del ANYmal. La condicion de
    exito de la fase 1 es que las 3 hayan sido desplazadas al menos
    _CLEARED_DIST metros desde su posicion inicial.
    """

    _CLEARED_DIST = 0.85   # metros que debe moverse una caja para estar "despejada"

    def __init__(self, box_positions=None, corridor=None):
        if box_positions is None:
            box_positions = [(5.0, 2.0), (6.0, 2.0), (8.0, 2.0)]
        self._initial_xy = [(float(x), float(y)) for x, y in box_positions]
        self.boxes = [Box(x, y, name=f"B{i+1}")
                      for i, (x, y) in enumerate(box_positions)]

        # Corredor: bounding box de las posiciones iniciales + margen.
        # Se usa solo para calcular el objetivo de empuje en plan_push.
        if corridor is not None:
            xmin, xmax, ymin, ymax = corridor
        else:
            xs = [p[0] for p in self._initial_xy]
            ys = [p[1] for p in self._initial_xy]
            pad = 0.70
            xmin = min(xs) - pad
            xmax = max(xs) + pad
            ymin = min(ys) - pad
    ...
    def box_in_corridor(self, box):
        """True si la caja aun no se ha desplazado lo suficiente."""
        for b, (ix, iy) in zip(self.boxes, self._initial_xy):
            if b is box:
                return math.hypot(b.x - ix, b.y - iy) < self._CLEARED_DIST
        return False
...
class HuskyPusher:
    ...
    # ------------------------------------------------------------------
    def clear_corridor_grid(self, transit_wps_per_box, line_rgb, dt=0.05,
                            box_order=None):
        """Despeja el corredor usando seguimiento de linea para los transits.

        Para cada caja, el Husky:
          1. Navega por cuadricula A* al punto de approach via linea amarilla.
          2. Ejecuta el push directo (igual que ``push_box``).

        Parameters
        ----------
        transit_wps_per_box : list of list-of-(x,y)
            Un elemento por caja (en orden ``box_order``). Cada elemento
            es la secuencia de waypoints A* (centros de celdas + approach
            exacto) para el transit al approach de esa caja.
        line_rgb : (R, G, B)
            Color de la linea del Husky en el suelo.
        dt : float  paso de integracion [s]
        box_order : list of str, opcional
            Nombres de caja en el orden de empuje. Si None, oeste→este.
        """
        from .line_follower import transit_to_waypoints

        log = self._empty_log(self.world.boxes)

        boxes_in = self.world.boxes_in_corridor()
        if box_order is not None:
            name_map = {b.name: b for b in boxes_in}
            boxes_to_push = [name_map[n] for n in box_order if n in name_map]
        else:
            boxes_to_push = sorted(boxes_in, key=lambda b: b.x)

        for box, transit_wps in zip(boxes_to_push, transit_wps_per_box):
            # Transit via seguimiento de linea
            if transit_wps and len(transit_wps) >= 2:
                transit_to_waypoints(
                    self.husky, transit_wps, line_rgb, dt, log,
                    world_boxes=self.world.boxes)

            # Push directo (mismo que push_box)
    ...
    # ------------------------------------------------------------------
    def clear_corridor(self, dt=0.05, box_order=None):
        """Empuja todas las cajas que esten dentro del corredor.

        Parameters
        ----------
        dt : float
            Paso de integracion [s].
        box_order : list of str, opcional
            Nombres de caja en el orden deseado de empuje (ej. ['B2','B1','B3']).
            Si es None, se ordena de oeste a este (por coordenada x).
        """
        log = self._empty_log(self.world.boxes)
        boxes_in = self.world.boxes_in_corridor()
        if box_order is not None:
            name_map = {b.name: b for b in boxes_in}
            boxes_to_push = [name_map[n] for n in box_order if n in name_map]
            # Anadir las que no aparezcan en box_order al final
            named = {b.name for b in boxes_to_push}
            extras = sorted([b for b in boxes_in if b.name not in named],
                            key=lambda b: b.x)
            boxes_to_push = boxes_to_push + extras
        else:
            boxes_to_push = sorted(boxes_in, key=lambda b: b.x)
