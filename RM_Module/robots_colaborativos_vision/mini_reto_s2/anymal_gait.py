...
# ---------------------------------------------------------------------------
#  Navegacion multi-waypoint (para seguimiento de cuadricula A*)
# ---------------------------------------------------------------------------
def simulate_anymal_waypoints(anymal, waypoints_xy,
                               gait=None, navigator=None,
                               T_max=120.0, dt=0.005,
                               start_xy=(0.0, 0.0), start_yaw=0.0,
                               singular_tol=1e-3):
    """ANYmal trota a traves de una secuencia ordenada de waypoints.

    Identico a ``simulate_anymal_to_target`` pero navega celda a celda
    por la trayectoria A*. En cada waypoint, el navegador elige el proximo.
    Esto hace que el ANYmal siga las lineas pintadas en el suelo en lugar
    de ir en linea recta al destino final.

    Parameters
    ----------
    waypoints_xy : list de (x, y)
        Secuencia de waypoints en orden (salida de
        ``compute_mission_paths``'s ``anymal_waypoints``).
    T_max : float
        Tiempo total maximo [s]. Se reparte entre todos los waypoints.
    """
    if gait is None:
        gait = ANYmalTrotGait(anymal)
    if navigator is None:
    ...
    waypoints = [np.asarray(wp, dtype=float) for wp in waypoints_xy]
    if not waypoints:
        raise ValueError("waypoints_xy no puede estar vacio")
    ...
    # Trim
    for k in ('t', 'base_x', 'base_y', 'base_yaw', 'v_cmd', 'omega_cmd', 'dist'):
        log[k] = log[k][:n_used]
    log['q'] = log['q'][:n_used]
    for name in ANYmal.LEG_NAMES:
        log['det_J'][name] = log['det_J'][name][:n_used]
    ...
    log['violations'] = violations
    log['final_pose'] = (anymal.base_pos[0], anymal.base_pos[1], anymal.base_yaw)
    log['final_error'] = float(np.hypot(
        anymal.base_pos[0] - waypoints[-1][0],
        anymal.base_pos[1] - waypoints[-1][1]))
    return log


# ---------------------------------------------------------------------------
#  Navegacion SOBRE el grid guiada por camara global (sin waypoints explícitos)
# ---------------------------------------------------------------------------
def simulate_anymal_grid_vision(anymal, goal_xy, aerial_cam, grid,
                                gait=None, navigator=None,
                                T_max=120.0, dt=0.005,
                                start_xy=(0.0, 0.0), start_yaw=0.0,
                                singular_tol=1e-3,
                                boxes=None):
    """ANYmal sigue el grid celda a celda guiado exclusivamente por la cámara.

    A diferencia de ``simulate_anymal_waypoints``, NO recibe lista de
    waypoints pre-computada.  En cada celda el coordinador consulta a la
    cámara aérea (``aerial_cam.next_step``) para obtener la dirección (dr,dc)
    del siguiente paso sobre el grid.  El movimiento es siempre cardinal
    (N/S/E/O) — nunca diagonal.

    Pipeline por celda:
      1. Renderizar escena actual (o recibir img) → cámara detecta posición
         y obstáculos.
      2. A* 4-vecinos sobre lo detectado → dirección siguiente (dr, dc).
      3. Calcular centro de la celda vecina → navegar hasta él con el gait.
      4. Repetir desde 1 hasta llegar a la celda objetivo.

    Parameters
    ----------
    aerial_cam  : AerialCamera  — cámara global que provee ``next_step``
    grid        : NavigationGrid
    boxes       : list of Box, opcional — para renderizar la escena
    goal_xy     : (x, y) destino en coordenadas mundo
    """
    from .aerial_camera import ANYMAL_RGB

    if gait is None:
        gait = ANYmalTrotGait(anymal)
    if navigator is None:
        navigator = ANYmalNavigator(stop_radius=0.20)
    ...
    log = {
        't':         [], 'base_x': [], 'base_y': [], 'base_yaw': [],
        'v_cmd':     [], 'omega_cmd': [], 'dist': [],
        'q':         [], 'det_J': {n: [] for n in ANYmal.LEG_NAMES},
        'foot':      {n: [] for n in ANYmal.LEG_NAMES},
    }
    violations = {n: 0 for n in ANYmal.LEG_NAMES}
    success = False
    t_global = 0.0
    step_budget = n_max

    while step_budget > 0:
        # ---- 1. Cámara analiza la escena y determina siguiente dirección ----
        r_cur = anymal.base_pos[0]; c_cur = anymal.base_pos[1]
        robots_pose = {'anymal': (r_cur, c_cur, anymal.base_yaw)}
        img = aerial_cam.render(grid, boxes, robots_pose)
        dr, dc = aerial_cam.next_step(img, ANYMAL_RGB, goal_cell, grid)

        if (dr, dc) == (0, 0):
            # En la celda destino o sin camino
            cur_cell = grid.world_to_cell(r_cur, c_cur)
            success = (cur_cell == goal_cell)
            break

        cur_cell = grid.world_to_cell(r_cur, c_cur)
        next_cell = (cur_cell[0] + dr, cur_cell[1] + dc)
        next_xy = np.asarray(grid.cell_center(*next_cell), dtype=float)

        # ---- 2. Navegar hasta la celda siguiente ----
        for _ in range(min(step_budget, int(8.0 / dt))):
            base_xy = anymal.base_pos[:2]
            v_cmd, omega_cmd, dist = navigator.compute_command(
                base_xy, anymal.base_yaw, next_xy)

            q_tgt, det_jacs, foot_tgt = gait.step(t_global, v_cmd)
            for name, q in q_tgt.items():
                anymal.legs[name].q = q
                if abs(det_jacs[name]) < singular_tol:
                    violations[name] += 1
            ...
            log['t'].append(t_global)
            log['base_x'].append(anymal.base_pos[0])
            log['base_y'].append(anymal.base_pos[1])
            log['base_yaw'].append(anymal.base_yaw)
            log['v_cmd'].append(v_cmd)
            log['omega_cmd'].append(omega_cmd)
            log['dist'].append(dist)
            log['q'].append(anymal.get_all_joint_angles().copy())
            for name in ANYmal.LEG_NAMES:
                log['det_J'][name].append(det_jacs[name])
                log['foot'][name].append(foot_tgt[name].copy())
    ...
    log['q'] = np.array(log['q']) if log['q'] else np.zeros((0, 12))
    for name in ANYmal.LEG_NAMES:
        log['det_J'][name] = np.array(log['det_J'][name])
        log['foot'][name]  = np.array(log['foot'][name]) if log['foot'][name] \
                             else np.zeros((0, 3))

    log['success']    = success
    log['violations'] = violations
    log['final_pose'] = (anymal.base_pos[0], anymal.base_pos[1], anymal.base_yaw)
    goal_arr = np.asarray(goal_xy, float)
    log['final_error'] = float(np.hypot(anymal.base_pos[0] - goal_arr[0],
                                        anymal.base_pos[1] - goal_arr[1]))
    return log
